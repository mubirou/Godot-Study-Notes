...
1. 上記のファイルを Web 上に置く（今回は **C:\xampp\htdocs** 内に配置し **http://local/TheRestaurantOfManyOrders.html** で閲覧できるようにする） 
